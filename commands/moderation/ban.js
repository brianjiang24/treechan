        ...
        if (user) {
            ...
            if (arguments[0] = member) {
                member.ban().then(async() => {
                    message.channel.send(Embed.setDescription(`Successfully banned ${user}`).setTimestamp());
                  ...
                  .catch(err => {
                    message.channel.send(Embed.setDescription('Unable to ban the user.').setTimestamp());
                    console.error(err);
                });
            } else {
